...
opt.modaltype = 'distillation'
log_name = str(curr_time) + "-" + zsl + "-" + "mars_core" + "-" + opt.modaltype + "-" + str(opt.dis) + "-" + str(opt.class_embedding) + "-" + "seen_unseen_ratio-" + str(opt.seen_unseen_ratio) + "-seed-" + str(opt.manualSeed) + "-batch_size-"  + str(opt.batch_size)
...
def main(): 
    ...
    semantic_zsl = prepare_semantic_label(class_semantic, data.unseenclasses).cuda() 
    semantic_seen = prepare_semantic_label(class_semantic, data.seenclasses).cuda()  
    semantic_gzsl = torch.transpose(class_semantic, 1, 0).cuda()  
    ...
    csv_train_name = './teacher_csvs/ZSMARS/' + zsl + "-mars_core-teacher-" + str(opt.class_embedding) + "-" + "seen_unseen_ratio-" + str(opt.seen_unseen_ratio) + "-seed-" + str(opt.manualSeed) + "-batch_size-"  + str(opt.batch_size) + '_tea_train_predictions' + '.csv'

    train_csv = pd.read_csv(csv_train_name)  

    train_csv = train_csv.values      
    train_tea_impath = train_csv[:, 0]
    ...
    if opt.class_embedding == "w2v":
        ...
        semantic = 'bert'
    reg_weight = {'final': {'xe': opt.xe, 'dis': opt.dis, semantic: opt.semantic, 'regular': opt.regular}}  
    ...
    with open(os.path.join("/work/tanxm/cache/", opt.dataset, "classindex2name.json"),"r") as f:
    ...
    if opt.only_evaluate:
        ...
        for epoch in range(opt.nepoch):  
            ...
            loss_log = {'ave_loss': 0, 'l_xe_final': 0, 'l_dis_final': 0, 'l_semantic_final': 0, 'l_regular_final': 0,
                        'l_xe_layer': 0, 'l_semantic_layer': 0, 'l_regular_layer': 0, 'l_cpt': 0}
            batch = len(trainloader)  

            trainpredict_class = []
            trainpredict_classpath = []
            ...
            for i_realindex, (batch_input, batch_target, impath, matrix, img_loc) in tqdm(enumerate(trainloader), total = len(trainloader)):
                ...
                tea_indices = [np.where(train_tea_impath == img_path) if img_path in train_tea_impath else None for img_path in impath] 

                arrays_to_concatenate = [tup[0] for tup in tea_indices]
     
                result = np.concatenate(arrays_to_concatenate) 

                if opt.seen_unseen_ratio == 64:
                    label_tea = train_csv[result][:,-6:]    
                elif opt.seen_unseen_ratio == 73:
                    label_tea = train_csv[result][:,-7:]
                else:
                    label_tea = train_csv[result][:,-8:]


                label_tea_float = label_tea.astype(np.float32) 
                opt.label_tea = torch.from_numpy(label_tea_float)

                if opt.cuda:
                    input_v = input_v.cuda()
                    label_v = label_v.cuda()
                    opt.label_tea = opt.label_tea.cuda()
                visual_label = []
                for index_impath in impath:
                    visual_label_index = int(str(index_impath).split("/")[5][0])        
                ...
                trainpredict_class.append(output) 
                trainpredict_classpath.append(impath)
                ...
                if ((opt.dataset == "SUN") and (((i_realindex + 1) == batch) or((epoch>=5) and (i_realindex +1) == 644)))  \
                    ...
                    or ((opt.dataset == "zsmars") and (((i_realindex + 1) == batch) or((epoch>=5) and (i_realindex +1) == batch/2/opt.gradient_time*opt.gradient_time))):
                    logger.info('\n[Epoch %d, Batch %5d] Train loss: %.3f '% (epoch+1, batch, loss_log['ave_loss'] / batch))
                    model_baseline.eval()

                    if not opt.gzsl:

                        acc_ZSL, avg_ZSL, testpredict_class = test_zsl(opt, model_baseline, testloader_unseen, semantic_zsl, data.unseenclasses, classindex2name)
                        ...
                        logger.info('\n[Epoch {}] ZSL test accuracy is {:.1f}%, avg_acc is {:.1f}%, Best_acc [{:.1f}% | Epoch-{}]'.format(epoch+1, acc_ZSL, avg_ZSL, result_zsl.best_acc, result_zsl.best_iter))
                    else:
                        acc_GZSL_unseen, avg_GZSL_unseen, testpredict_class = test_gzsl(opt, model_baseline, testloader_unseen, semantic_gzsl, data.unseenclasses, classindex2name, opt.calibrated_stacking)
                        acc_GZSL_seen, avg_GZSL_seen, testpredict_class = test_gzsl(opt, model_baseline, testloader_seen, semantic_gzsl, data.seenclasses, classindex2name, opt.calibrated_stacking)
                        if (acc_GZSL_unseen + acc_GZSL_seen) == 0:
                            acc_GZSL_H = 0
                        else:
                            acc_GZSL_H = 2 * acc_GZSL_unseen * acc_GZSL_seen / (acc_GZSL_unseen + acc_GZSL_seen)
            ...
            if epoch == result_zsl.best_iter:
                if isinstance(trainpredict_class, list):
                    trainpredict_class = torch.cat(trainpredict_class, dim=0) 
                trainpredict_class_np = trainpredict_class.detach().cpu().numpy()
                print('trainpredict_class:', trainpredict_class_np.shape)
                trainpredict_classpath_1 = [pathim for sublist in trainpredict_classpath for pathim in sublist]
                len_trainpredict_classpath = len(trainpredict_classpath_1)   
                array_trainpredict_classpath = np.array(trainpredict_classpath_1).reshape(len_trainpredict_classpath, 1)
                print('array_trainpredict_classpath:', array_trainpredict_classpath.shape)

                predictclass_result_test = testpredict_class.cpu()
                if isinstance(predictclass_result_test, torch.Tensor):
                    predictclass_result_test_np = predictclass_result_test.detach().numpy()
