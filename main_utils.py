...
def test_zsl(opt, model_baseline, testloader, semantic, test_classes, classindex2name):
    ...
    if opt.all:
        return acc_all * 100, acc_avg * 100, predictclass_result
    else:
        return acc_all * 100, acc_avg * 100, predictclass_result
...
def test_gzsl(opt, model_baseline, testloader, semantic, test_classes, classindex2name, calibrated_stacking_number):
    ...
    with torch.no_grad():
        for i, (input, target, impath, matrix, img_loc) in tqdm(enumerate(testloader), total = len(testloader)):
            ...
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            output_class, output_attr, mask_feature, pre_class,loss_mask, _,_= model_baseline(input, semantic, visual_label_unseen, is_mask=False)
            predictclass_result = torch.cat((predictclass_result, output_class), dim=0)  
    ...
    if opt.all:
        ...
        return acc_avg * 100, acc_avg * 100, predictclass_result
...
def Loss_fn(opt, loss_log, reg_weight, criterion, criterion_regre, model, output, pre_semantic, label_a, label_v, embedding_for_sc):
    ...
    if opt.modaltype == 'distillation':
        soft_loss = nn.KLDivLoss(reduction='batchmean')
        loss_dis = soft_loss(F.log_softmax(output/opt.temperature, dim=-1), F.softmax(opt.label_tea/opt.temperature, dim=-1))
        loss_log['l_dis_final'] += loss_dis.item()
        loss = reg_weight['final']['dis'] * loss_dis + (1-reg_weight['final']['dis']) * loss
