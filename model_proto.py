...
from core_bert import CustomBertModel
import cotlay
...
class Multi_attention_Model(nn.Module):
    def __init__(self, opt, using_amp =False):
        ...
        self.bert = CustomBertModel(student_config)
        self.tokenizer = BertTokenizer.from_pretrained("/work/tanxm/plms/bert-base-uncased", do_lower_case=True)
    ...
    def forward(self, x, seg_features, texts, whole_semantic=None, con_labels=[], batch_target=None,semantic_deal=None,do_predict=False,texts_label=[],impath=[],texts_label_withpro=[]):

        if self.modaltype == "teacher":
            image_embedding = self.linear(self.deit(x).logits).unsqueeze(1)    
        else:
            image_embedding = self.linear(self.deit(x)).unsqueeze(1)
