...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        self.label = matcontent['visual_classids'].astype(int).squeeze()   
        self.image_files = matcontent['visual_imagefiles'].squeeze()
        self.image_ids = matcontent['visual_imageids'].astype(int).squeeze()       

        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + "splits.mat")
          
        if opt.gzsl == "gzsl":
            if opt.seen_unseen_ratio == 64:
                    self.seen_medloc = matcontent['seen_loc1'].squeeze()    
   
                    self.seen_maskinx = np.isin(self.image_ids, self.seen_medloc)
  
                    self.seen_loc = np.where(self.seen_maskinx)[0]                 
         
                    self.train_loc = self.seen_loc    
                    self.unseen_medloc = matcontent['unseen_loc1'].squeeze()     
                    self.unseen_maskinx = np.isin(self.image_ids, self.unseen_medloc)
                    self.unseen_loc = np.where(self.unseen_maskinx)[0]
                    self.test_seen_loc = self.seen_loc    
                    self.test_unseen_loc = self.unseen_loc
            elif opt.seen_unseen_ratio == 73:
                    self.seen_medloc = matcontent['seen_loc2'].squeeze()    
                    self.seen_maskinx = np.isin(self.image_ids, self.seen_medloc)
                    self.seen_loc = np.where(self.seen_maskinx)[0]  
                    self.train_loc = self.seen_loc     
                    self.unseen_medloc = matcontent['unseen_loc2'].squeeze()    
                    self.unseen_maskinx = np.isin(self.image_ids, self.unseen_medloc)
                    self.unseen_loc = np.where(self.unseen_maskinx)[0]
                    self.test_seen_loc = self.seen_loc                   
                    self.test_unseen_loc = self.unseen_loc                
            elif opt.seen_unseen_ratio == 82:
                    self.seen_medloc = matcontent['seen_loc3'].squeeze()    
                    self.seen_maskinx = np.isin(self.image_ids, self.seen_medloc)
                    self.seen_loc = np.where(self.seen_maskinx)[0]                     
                    self.train_loc = self.seen_loc    
                    self.unseen_medloc = matcontent['unseen_loc3'].squeeze()    
                    self.unseen_maskinx = np.isin(self.image_ids, self.unseen_medloc)
                    self.unseen_loc = np.where(self.unseen_maskinx)[0]                    
                    self.test_seen_loc = self.seen_loc                  
                    self.test_unseen_loc = self.unseen_loc 
        ...
        self.seg_class = matcontent['seg_class'].squeeze()      #(10,) 
...
def default_flist_reader(opt, image_files, img_loc, image_labels, dataset):
    ...
    if opt.dataset == 'zsmars':
        for image_file, image_label, img_lo in zip(image_files, image_labels, img_loc):
            ...
            image_medfile = str(image_file).split("zsmars")[1].replace("\\\\","/")  


            image_name = image_medfile.split("/")[2].replace("']"," ").rstrip()
            image_file = opt.image_root + opt.dataset + "/" + image_medfile.split("/")[1] + "/" + image_name
